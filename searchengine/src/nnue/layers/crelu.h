...
void crelu32(const int32_t* input, int8_t* output) {
    ...
    for (size_t i = 0; i < numChunks; ++i) {
        // load the 4 input registers and pack them into 16 bit elements
        // order: [3, 2] [1, 0] -> [3, 1, 2, 0]
        __m256i in0 = _mm256_packs_epi32(
            _mm256_load_si256((const __m256i*)&input[(i*4 + 0)*inRegisterWidth]),
            _mm256_load_si256((const __m256i*)&input[(i*4 + 1)*inRegisterWidth])
        );
        // order: [7, 6] [5, 4] -> [7, 5, 6, 4]
        __m256i in1 = _mm256_packs_epi32(
            _mm256_load_si256((const __m256i*)&input[(i*4 + 2)*inRegisterWidth]),
            _mm256_load_si256((const __m256i*)&input[(i*4 + 3)*inRegisterWidth])
        );
        ...
        __m256i out = _mm256_permutevar8x32_epi32(
            ...
            _mm256_max_epi8(
                ...
                // this saturates the values to 127
                // permutes the elements:
                // [7, 5, 6, 4] [3, 1, 2, 0] -> [7, 5, 3, 1, 6, 4, 2, 0]
                // idxs                          7  6  5  4  3  2  1  0
                // the control correctly reorders them
                _mm256_packs_epi16(in0, in1), 
                zero),
            control
        );
